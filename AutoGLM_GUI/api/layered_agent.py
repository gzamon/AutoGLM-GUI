...
async def layered_agent_chat(request: LayeredAgentRequest) -> StreamingResponse:
    ...
    async def event_generator() -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            yield f"data: {json.dumps({'type': 'status', 'phase': 'thinking', 'message': '决策层开始思考任务...'}, ensure_ascii=False)}\n\n"
            ...
            if "connection" in error_message.lower() or "timeout" in error_message.lower():
                error_type = "network"
                error_suggestion = "请检查网络连接，或尝试使用国内模型服务。"
            elif "api key" in error_message.lower() or "unauthorized" in error_message.lower():
                error_type = "auth"
                error_suggestion = "API Key 无效，请检查配置。"
            elif "rate limit" in error_message.lower():
                error_type = "rate_limit"
                error_suggestion = "请求过于频繁，请稍后重试。"
            elif "model" in error_message.lower() and ("not found" in error_message.lower() or "does not exist" in error_message.lower()):
                error_type = "model"
                error_suggestion = "模型不存在，请检查模型名称配置。"
            elif "device" in error_message.lower() and ("busy" in error_message.lower() or "disconnect" in error_message.lower()):
                error_type = "device"
                error_suggestion = "设备连接异常，请重新连接设备。"
