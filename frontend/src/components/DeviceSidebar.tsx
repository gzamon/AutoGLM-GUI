...
const getSavedConnectionInfo = (): { ip: string; port: string } => {
  try {
    const saved = localStorage.getItem('wifi-connection-info');
    if (saved) {
      return JSON.parse(saved);
    }
  } catch (error) {
    console.warn('Failed to load saved connection info:', error);
  }
  return { ip: '', port: '5555' };
};

const saveConnectionInfo = (ip: string, port: string) => {
  try {
    localStorage.setItem('wifi-connection-info', JSON.stringify({ ip, port }));
  } catch (error) {
...
export function DeviceSidebar({
  ...
  const savedConnectionInfo = getSavedConnectionInfo();
  const [manualConnectIp, setManualConnectIp] = useState(savedConnectionInfo.ip);
  const [manualConnectPort, setManualConnectPort] = useState(savedConnectionInfo.port);
  ...
  const handleManualConnect = async () => {
    ...
    try {
      ...
      if (result.success) {
        saveConnectionInfo(manualConnectIp, manualConnectPort);
  ...
  const handlePair = async () => {
    ...
    try {
      ...
      if (result.success) {
        saveConnectionInfo(manualConnectIp, connectionPort);
        setShowManualConnect(false);
        // Reset form
        setPairingCode('');
        setPairingPort('');
